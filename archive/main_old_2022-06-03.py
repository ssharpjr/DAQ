...
#     """
#     return redis.Redis(host=CONST_REDIS_HOST,port=CONST_REDIS_PORT)

# def util_redis_queue_device_message(
#     param_redis_conn,
#     param_device_id,param_data):
#     """
#     queue a device's message for parsing and
#     DB persistence

#     Args:
#         :param param_redis_conn:
#         :param param_device_id|str: the device id from db
#         :param param_data|str: messages from device
#     Returns:
#         :rtype|void:
#     """

#     param_redis_conn.rpush(
#         param_device_id,
#         str(param_data)
#     )

# def util_redis_get_device_message(
#     param_redis_conn,
#     param_device_id):
#     """
#     get a device's message for parsing and
#     DB persistence
...
#     return param_redis_conn.lpop(param_device_id)

# def util_redis_count_device_messages(
#     param_redis_conn,
#     param_device_id):
#     """
...
#     return param_redis_conn.llen(param_device_id)
...
class Db_Table_Product(VAR_SQL_ALCHEMY_DECLARATIVE_BASE):
    ...
    @classmethod
    def get_cat_id_by_product_id(cls,param_sqlalch_session):
        data = param_sqlalch_session.query(cls).filter(
            cls.ProductID == Db_Table_Finished_Good.ProductID
        ).first()
        return data
...
def app_parse_calc_plastic_produced(
    ...
    """
    if (param_actual_lay_flat==None) or (param_finished_good_width==None):
        return None
...
# def app_consume_messages(param_consumer_id, param_messge_data):
#     """
#     consume messages in queue,parse and save

#     Args:
#         :param param_consumer_id|int: id/index for this consumer
#         :param param_messge_data|str: data for the consume id
#     Returns:
#         :rtype|void:
#     """
...
#     #get devices
#     db_data_production_devices = Db_Table_Production_Devices.get_by_ids(
#         db_conn_session,
#         READ_DEVICES
#     )

#     #start telnet client threads
#     for device in db_data_production_devices:
#         all_devices_ids.append(device.deviceID.strip())

#     util_log(
#         "STARTED CONSUMER",
#         "INDEX ==> {}".format(param_consumer_id),
#         param_oneline=True
#     )

#     while True:
#         # time.sleep(CONST_CONSUMER_INTERVAL)

#         #read device id message queues
#         for device_id in all_devices_ids:
#             try:
#                 #get message
#                 earlist_message = param_messge_data
#                 # device_messages_count = util_redis_count_device_messages(
#                 #     redis_conn,
#                 #     device_id
#                 # )
#                 util_log(
...
#                     "MESSAGE RETEIEVED ==> {}".format(earlist_message),
#                     param_oneline=True
#                 )

#                 #parse message
#                 if earlist_message not in [None,""]:
...
def app_telnet_connect_and_read(
    ...
    #param_device_read_interval,
    #param_device_read_timeout,
    ...
    while True:
        ...
        print("\n\n*** BEGIN TELNET READ LOOP (telnet read interval: " + str(telnet_read_interval) + ") ***\n")
        
        time.sleep(telnet_read_interval)
        # print("\n\n*** DEBUG ***")
        # print("telnet_read_interval: " + str(telnet_read_interval))
        # print("*************\n\n")
            ...
            while True:
                if telnet_obj is not None:
                    ...
                    if str(telnet_data).find("V0") != -1:
                        ...
                        # Process the queued data and save it 
                        # app_consume_messages(param_device_id, telnet_data)


                        # util_redis_queue_device_message(
                        #     redis_conn,
                        #     param_device_id,
                        #     telnet_data
                        # )
...
def app_start_listeners():
    ...
    # all_listener_threads = []

    # open db connection
    db_conn_engine,db_conn_session = util_db_open_conn_sess(CONST_DATABASE_URL)

     #get settings
    # db_data_settings = Db_Table_Settings.get_latest(db_conn_session)
    ...
    #             db_data_settings.Sample_time,
    #             db_data_settings.telnet_timeout)
    ...
    # #start telnet client threads
    # for device_index,device in enumerate(db_data_production_devices):
    #     th = threading.Thread(
    #         target=app_telnet_connect_and_read,
    #         args=(
    #             device.deviceID.strip(),
    #             CONST_DEFAULT_TEST_SERVERS_DEFAULT_ADDRESS if CONST_ENV_TEST else device.extruderIP.strip(),
    #             db_data_settings.Sample_time,
    #             db_data_settings.telnet_timeout,
    #             device_index
    #         ),
    ...
    #     )
    #     th.start()
    #     all_listener_threads.append(th)



    # return all_listener_threads

# def app_start_consumers():
#     """
#     start a consumer to read queued messages
#     parse it and save to messages

#     Returns:
#         :rtype|list: list of threads
#     """

#     all_consumer_threads = []

#     for consumer_index in range(1):
#         th = threading.Thread(
#             target=app_consume_messages,
#             args=(
#                 str(consumer_index)
#             ),
#             daemon=True
...
#         all_consumer_threads.append(th)

#     return all_consumer_threads

#**************************************************************
#start thread to parse read data
#and save to db
#**************************************************************
...
def run():
    ...
    app_start_listeners()
    # all_threads.extend(threads_listners)

    # threads_consumers = app_start_consumers()
    # all_threads.extend(threads_consumers)

    # util_log(
    #     "THREADS STARTED",
    #     "TOTAL ==> {}".format(len(all_threads)),
    #     param_oneline=True
    # )
