...
def ping_device(param_device_id, param_device_address):
    ...
    resp = ping(param_device_address, count=1, timeout=3)

    # if not ping_device == 0:
    if resp.success() == False:
        count = 1
        print("Telnet service on {} is not responding. Restarting app in " + str(count) + " second(s)...".format(param_device_id))
        time.sleep(count)
        print("Restarting DAQ service for " + param_device_id + "...")
        os.system("sudo systemctl restart daq-" + param_device_id.lower() + ".service")
        sys.exit()
    else:
        print("\nTelnet service on {} is up. Continuing...".format(param_device_id))
