...
def parse_telnet_data(param_data, single_run=None):
    ...
    if not single_run:
        # Update exc and exe2 from global
        global EXC_BOX_LAST
        global EXC_DB_LAST
...
def app_parse_calc_plastic_produced(
    ...
    if param_cat_id == 363 or param_cat_id == 374 or param_cat_id == 559:
        ...
        # Check if roll is larger than lay flat, jut return a 1
...
def app_set_global_extruder_counts():
    ...
    # # Reset counts if less than zero
    # if exc_db_last <= 0:
    #     exc_db_last = 0
    # if exe2_db_last <= 0:
    #     exe2_db_last = 0
