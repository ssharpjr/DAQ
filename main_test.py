...
def app_parse_and_save_data(
    ...
    if current_daq_box_exe2 >= max_db_exe2:
        ...
        if LOGGING:
            logging.debug("+ EXE2_DELTA: {} ( = current daq box exe2) [REBOOT DETECTED]".format(exe2_delta))
    ...
    if current_daq_box_mnc >= max_db_mnc:
        ...
        if LOGGING:
            logging.debug("+ MNC_DELTA: {} ( = current daq box mnc) [REBOOT DETECTED]".format(mnc_delta))
